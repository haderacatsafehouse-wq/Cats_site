...
כעת כולל תמיכה ב"קישור חתולים" (Cat Linking) — שיוך חתול לחתולים אחרים.
...
- סכימה בסיסית: טבלאות `locations`, `cats`, `media` וטבלאות תגיות (many-to-many), וכן טבלת `cat_links` לקישורים סימטריים בין חתולים. מופעל `PRAGMA foreign_keys = ON`.

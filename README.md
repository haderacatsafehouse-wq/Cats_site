...
- `inc/`
  ...
  - `db.php` — חיבור SQLite, סכימה ופונקציות DAO (חתולים, מדיה, מיקומים, תגיות).
...
- `admin/search_cats.php`
  - Query params: `q` (חיפוש טקסט חופשי בשם/תיאור/מיקום), `limit` (ברירת מחדל 200).
  - פלט: `{ success: true, items: [ { id, name, location_name, thumb_url } ] }`.
  - תמונות ממוזערות נוצרות ע"י טרנספורמציה של Cloudinary לשיפור ביצועים.
...
- פרמטרים ב-URL: 
  ...
  - `tag` — סינון לפי תגית ספציפית (ללא קשר ל-`q`; אם `q` קיים הוא עדיף ומחזיר לפי חיפוש חופשי).
- דוגמה: `/index.php?q=ג׳ינג׳י חמוד&tag=בוגר`
...
- סכימה בסיסית: טבלאות `locations`, `cats`, `media` וטבלאות תגיות (many-to-many). מופעל `PRAGMA foreign_keys = ON`.
